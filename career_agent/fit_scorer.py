...
class FitScorer:
    ...
    def score_job(self, job: JobListing) -> FitScoreResult:
        ...
        if ai_matches:
            ...
            deduct_reason = "Full 30 pts awarded: 88,000+ LOC microservices, 455/456 passing tests, and system steering prompt profile." if ai_score == 30 else f"Awarded {int(ai_score)}/30 pts. 88k LOC + 455 unit tests verified. Deducted {30-int(ai_score)} pt(s) for specific niche tech stack variants in job spec."
            audit_details["AI Engineering & Code"] = {
                "score": f"{int(ai_score)}/30 pts",
                "target_requirement": f"Hands-on AI systems architecture, agentic orchestration, Python microservices ({', '.join(ai_matches[:3])}).",
                "candidate_receipt": "88,000+ lines of production code across BrijStream & Kyvryn; 455 passing unit tests out of 456; author of System Steering prompt architecture.",
